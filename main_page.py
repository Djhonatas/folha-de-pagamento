...
def calcular_irrf(salario_liquido):
  if salario_liquido <= 1909.98:
    ...
    return (salario_liquido * 0.225) - 869.36


def calcular_hora_extra(salario_bruto, horas_extras_trabalhadas):
  valor_hora = salario_bruto / 220 #horas trabalhadas por mês
  adicional = valor_hora *0.5
  return horas_extras_trabalhadas * (valor_hora + adicional)

salario_bruto = float(input("Digite o salário bruto do trabalhador: R$"))
horas_extras = int(input("Digite a quantidade de horas extras trabalhadas no mês: "))
...
print(f"Provento horas extras: R${provento_horas_extras:.2f}")
